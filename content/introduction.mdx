...
<Resources 
    text="In the original Bitcoin, these conditions were to  use a consensus protocol which dictated that each member of the network agreed to use the longest network with a valid cryptographic history, and to allow the first member to solve a cryptographic puzzle to add one block." 
    resources={[
...
<p style={{ textAlign: 'center' }}>Figure 1: Visualization of Proof of Work</p>
