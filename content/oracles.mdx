...
1. What degree of BFT does the oracle have?
  ...
  * What would it cost an actor to push specific data through that oracle?

2. What is the ultimate source of the data? (Company? Web feed? Sensors?)
2. What is the cost of getting data?
2. What is the delay in getting data?
2.
...
The alternative to centralized oracles is decentralized oracles. Decentralized oracles leverage input from lots of different sources to determine the truths that are reported. This category expands in a few different ways. For example:
...
These different membership methods can also be broken down with different solutions:

1. Hardware devices with sensors that report their information.

1. Servers that fetch data from the internet.
...
It is difficult to go into too much detail here, because there is so much innovation possible in this area. Please refer to the suggested reading to understand how some of these systems might be implemented, and to explore the space more.
