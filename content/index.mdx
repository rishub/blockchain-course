...
//Find scalability/measurement study from Eyal et al (or maybe it was EGS?) that looks into this as more detail, specifies what numbers work for best participation vs scalability
...
#### Point of Interest: There are a TON of trade-offs in Blockchain
One key thing we want to emphasize here is that many things in blockchain are trade-offs. These trade-offs exist throughout the field, and they are also one of the best ways to evaluate the strength of a given blockchain project

Note from James: whenever I read a new white-paper or listen to a new blockchain idea, I try to evaluate what technical, theoretical, and trust trade-offs are being made to enable whatever benefits are claimed: Bitcoin is generally a good reference point for those comparisons

##### Sidechains? Ripple? Etc? Can go more into detail on these. Providing raw research/word vomit for these here.

##### Sidechains
Sidechains independently handle transactions and regularly “settle up” with the main blockchain.
//image of sidechain
As you see here, the “main blockchain” could be something like Bitcoin. Bitcoin has fairly high transaction costs, which can only transmit a limited amount of information. This does not scale to everyday transactions: a single high-traffic retailer such as Starbucks would quickly overwhelm the system. If you create a sidechain, however, you can interlink an independent system with higher throughput into the network.
...
- Can be customized without losing guarantees
Bad
- (more) Susceptible to vulnerabilities
- May not have an open payment channel - must go through network
...
Sidechains offer superior throughput compared to a normal blockchain, significantly less latency (because you are not waiting for blocks to process in between transactions, you just need your IOUs to propagate the sidechain), and far lower fees (however, there is a larger fee, ~$4 when I checked, to set up a bidirectional payment channel on the Lightning network - other sidechains could bring this down.)
...
The leader election system is pretty similar: You have to complete a proof-of-work puzzle to lead. You get rewarded for solving the puzzle and leading. You get 60% of the fees from the previous block (to incentivize honesty - the paper has the analysis of what guarantees this provides - in essence, it allows more data to propagate, with stronger guarantees for network health), and 40% of the fee from the block you’re currently leading.
...
Current Bitcoin-NG implementations boast ~100tx/second. At its core, the original Bitcoin-NG paper simply highlighted stronger guarantees (fairness, BFT, mining power utilization) with equivalent tx/sec. This allows for increasing the block size to achieve higher throughput while maintaining decentralization and reducing the risk of malicious control.

10-40 seconds depends on block size

I couldn’t find an estimate of the transaction fee - this system has not been well-adopted - but it would be less than bitcoin simply due to the throughput, as it keeps a lot else equal to Bitcoin.

Medium waiting time

##### Ripple, Private/Permissioned Blockchains
*Ripple*
...
The numbers here are quite impressive: the best we’ve seen yet. However, they all need to be asterisked: this network does not maintain the same guarantees around decentralization and trustlessness that public blockchains provide. This means that you can achieve high throughput, but only with a smaller group of trusted partners (e.g. a network of national banks, a limited network of international banks, a combination of tech companies, etc), which will inherently be less useful to public, individual users (and function more like a traditional product/service). Instead of comparing this with public blockchains, it should really be compared with other inter-organization settlement options.

*: Ripple is a permissioned blockchain. This reduces the usefulness for individual users, and is mostly beneficial for inter-organization products.
